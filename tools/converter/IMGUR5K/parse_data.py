...
def get_train_val_test_indices(args):
    ...
    for split in SPLITS:
        # breakpoint()
        split_idx[split] = np.loadtxt(
            str(args.paths_to_splits[split]),
            delimiter="\t",
            dtype=np.str_,
            encoding="UTF-8",
        )
...
def rotate_crop_image(args, img, sub_box):
    ...
    crop_img = rotated_img[
        max(0, int(yc - h / 2)) : int(yc + h / 2),
        max(0, int(xc - w / 2)) : int(xc + w / 2),
    ]
...
def process_data(args, data, split_idx):
    ...
    for img_name in img_to_process:
        ...
        if args.verbose >= 1:
            print(
                f"Processing {'/'.join([args.path_to_repo, img_path])} into *{split}* set. Contains {len(sub_images_lst)} sub-image(s)."
            )
        ...
        # Read image.
        img = cv2.imread("/".join([args.path_to_repo, img_path]))

        # Check if the image file exists.
        if img is None:
            print(f"* {'/'.join([args.path_to_repo, img_path])} does not exist.")
            continue
        ...
        for sub_image in sub_images_lst:
            ...
            cv2.imwrite(str(args.paths_out[split] / out_filename), crop_img)
...
def main():
    ...
    args.path_to_annotations = Path(args.path_to_repo) / "dataset_info/imgur5k_annotations.json"
    # Structure:
    ...
    args.paths_to_splits = {split: Path(args.path_to_repo) / f"dataset_info/{split}_index_ids.lst" for split in SPLITS}
    args.paths_out = {
        split: Path(args.path_to_repo) / f"{args.out}_{split}_{args.idx_from}_{args.idx_to}/test/" for split in SPLITS
    }
    args.paths_to_gt = {
        split: Path(args.path_to_repo) / f"{args.out}_{split}_{args.idx_from}_{args.idx_to}/gt.txt" for split in SPLITS
    }
